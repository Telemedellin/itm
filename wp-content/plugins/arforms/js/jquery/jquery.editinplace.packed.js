...
$.extend(InlineEditor.prototype, {
	...
	handleSaveEditor: function(anEvent) {
		...
		jQuery('#arfname_'+fi_id).val(enteredText);
            jQuery('#ar_op_lable_' + fi_id).val(enteredText);
            jQuery('#ar_op_value_' + fi_id).val(enteredText);
            
            //console.log('#fieldcheck_' + fi_id);
            //var op_li_id = fi_id.split('key_');
            ///console.log(op_li_id[1]);
            
            var data_fid = this.dom.attr("data-fid");
            var data_op_key = this.dom.attr("data-op-key");
            if (jQuery('#separate_value_' + data_fid).is(':checked')) {
                var fieldcheck_val = jQuery('#ar_op_value_' + fi_id).val(); 
            } else {
                var fieldcheck_val = jQuery('#ar_op_lable_' + fi_id).val(); 
            }
            
            if (typeof (fieldcheck_val) != 'undefined') {
                
                jQuery('#fieldcheck_' + data_fid).val(fieldcheck_val);
                jQuery('#arf_op_fieldcheck_sub_' + data_fid+'-'+data_op_key).val(fieldcheck_val);
                jQuery('#fieldcheck_' + data_fid+'-'+data_op_key).val(fieldcheck_val);
                
            }
            
            //console.log('ar_op_value_' + fi_id);
	...
	isUnchangedInput: function(enteredText) {
		var class_name = this.dom.attr("class"); 
            ...
            if (class_name.indexOf('arfoptioneditorfield') >= 0 ||
                    class_name.indexOf('arfeditorformname') >= 0 ||
                    class_name.indexOf('arfeditorformdescription') >= 0)
            {
            ...
            if (class_name.indexOf('arf_op_not_send_ajax') >= 0) {
                this.originalValue = enteredText;
            }
