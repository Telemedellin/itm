...
jQuery( function( $ ){
	...
	$( 'body' ).on( 'click', '.wmts_search_submit', function( e ){
		...
		wmts_ajax( 'wmts_search', $this, data, wmts_search_beforeSend, wmts_search_complete );
	} )
	...
	$( 'body' ).on( 'click', '.wmts_pagination a, .wmts_pagination span, .wmts_pagination .wph_pagination_load_more', function( e ){
		...
		if( $this.is( '.wph_pagination_load_more' ) ){
			var 	current_pagination = Math.max( $this.attr( 'data-wph-paged' ), 1 ),
					data = { current_pagination: ( parseInt( current_pagination ) + 1 ) };
			wmts_ajax( 'wmts_pagination', $this, data, wmts_pagination_beforeSend, wmts_pagination_complete );
			return;
		}
	...
	function wmts_pagination_complete( $this ){
		$this.closest( '.wmts_container' ).find( '.wmts_members' ).removeClass( 'wmts_loading' );
	}
	...
	function wmts_ajax_success_callback( $showcase, response ){
		...
		// case of append
		if( typeof $showcase.data( 'wmts-overall-settings' )[ 'pagination' ] !== 'undefined' && $showcase.data( 'wmts-overall-settings' )[ 'pagination' ] === 'Enabled' && $showcase.data( 'wmts-overall-settings' )[ 'pagination_type' ] === 'load_more' ){
		...
		if( response.markup.filters ){
			...
			// case of replace, ie, load more button
			if( typeof $showcase.data( 'wmts-overall-settings' )[ 'pagination' ] !== 'undefined' && $showcase.data( 'wmts-overall-settings' )[ 'pagination' ] === 'Enabled' && $showcase.data( 'wmts-overall-settings' )[ 'pagination_type' ] === 'load_more' ){
				
				if( $showcase.find( '.wph_pagination_load_more' ).attr( 'data-wph-paged' ) === 1 ){
					$filters_container.html( response.markup.filters );					
				}
				
				$filters_container.append( response.markup.filters );
				// remove duplicates
				$( $filters_container.find( '.wph_filter' ).get( ).reverse( ) ).each( function( ){
					var 	$this = $( this ),
							filter = $this.attr( 'data-wph-filter' );
					if( $this.siblings( '[data-wph-filter="'+ filter +'"]' ).length ) $this.remove( );
				} );
				// console.log( 'load more pagination' );
			// case of replace, ie, numbered pagination
			}else{
				$filters_container.html( response.markup.filters );
				// console.log( 'numbered pagination pagination' );
			}
		...
		if( ! $showcase.find( '.wmts_member' ).length ){
			...
			$( '.wmts_clear_search_trigger', $showcase ).parent( ).remove( );
		}
	...
	wmts.showcase.init = function( $showcase ){
		...
		$showcase.magnificPopup( {
				delegate: "[href='lightbox'], [data-wph-href='lightbox']", // the selector for gallery item
				gallery: {
						enabled:true
					},
				callbacks: {
		...
		$showcase.on( 'click.wmts_filtering', '.wmts_filters span', function( ){
			var 	$this = $( this ),
					filter = $this.attr( 'data-wph-filter' ),
					$showcase = $this.closest( '.wmts_container' );
			$this.siblings( ).removeClass( 'wmts_selected_filter' );
			$this.addClass( 'wmts_selected_filter' );
			if( filter === '*' ){
				$showcase.children( '.wmts_members' ).isotope( { filter: '*' } );
			}else{
				$showcase.children( '.wmts_members' ).isotope( { filter: function( ){
					var 	$this = $( this );
							elm_filter = $this.attr( 'data-wph-filter' );
					if( ! elm_filter ) return;
					var elm_filters = $showcase.attr( 'data-wmts-version' ) ? elm_filter.split( '| ' ) : elm_filter.split( ', ' );
					if( $.inArray( filter, elm_filters ) !== -1 ) return true;
				} } );
			}
		} )
		if( ! $showcase.find( '.wmts_filters span.wmts_selected_filter' ).length )
			$showcase.find( '.wmts_filters span' ).first( ).click( );
		var 	selected_filter = $showcase.children( '.wmts_filters span.wmts_selected_filter' ),
				click_filter = selected_filter.length ? selected_filter : $showcase.children( '.wmts_filters span:first-child' );
		click_filter.click( );
	...
	wmts.showcase.resize = function( $showcase ){
		...
		// member setting
		$members_container.children( '.wph_element' ).outerWidth( columnWidth ).css( {'margin-bottom': device_settings.cellSpace, 'fontSize': device_settings.fontSize +'px' } );
		// height calc
		if( device_settings.height_calc === 'Aspect based' ){
			$members_container.children( '.wph_element' ).outerHeight( rowHeight );
		}else{
			$members_container.children( '.wph_element' ).height( '' );
		}
		...
		$images.each( function( ){
			...
			$this.centerImage( type, function( ){
				...
				$this.siblings( 'canvas' ).addBack( ).removeClass( 'wmts_isotope_arranging' );
...
//		var attrs = [ 'data-wmts-template', 'data-wmts-query', 'data-wmts-overall-settings', 'data-wmts-devices-settings' ];
		var attrs = [ 'data-wmts-overall-settings', 'data-wmts-devices-settings' ];
		// covert attributes to data
		$.each( attrs, function( i, attr ){
			var object = JSON.parse( $showcase.attr( attr ) || '{ }' );
