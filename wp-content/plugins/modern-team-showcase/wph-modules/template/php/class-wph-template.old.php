...
class WPH_Template{
	...
	function return_markup( $query, $query_handler, $template, $modifier= false, $post_id, $overall_settings ){
		...
		if( $the_query->have_posts( ) ){
			...
			$members = phpQuery::newDocumentHTML( );
			$elm_id = 0; // id of template element
			$css = '<style type="text/css">';
			
			while ( $the_query->have_posts( ) ){
				$the_query->the_post( );
				// create, append and target a new member
				$members->append( '<div class="xxx"></div>' );
				$member_container = $members->children( '.xxx' )->filter( ':last' );

				// build the member
				$selector_prepend = array( 'container' => '[data-wph-post-id="'. $post_id .'"]', 'parent' => null );
				$this->traverse_and_build_elements( $template, $member_container, $css, $elm_id, $selector_prepend );
				$member = $member_container->children( )->children( );
				...
				if( $filtering_key ){
					...
					if( $filter_val ){
						...
						$member->attr( 'data-wph-filter', $filter_val_markup ); // add filter to member markup
			...
			if( count( $filters ) ){
				...
				if( ! empty( $overall_settings[ 'filtering_details' ] ) ){
					...
					$new_filters = array( );
//					echo json_encode( $filtering_details );
//					echo '<br>';
...
//							$new_filters[ $user_filter[ 'name' ] ] = str_replace("&quot;", '"', $user_filter[ 'content' ]);
							$new_filters[ $user_filter[ 'name' ] ] = $user_filter[ 'content' ];
						}
					}
					
//					echo json_encode( $new_filters );
			...
			$markup = array( 'members'=> $css . $members->html( ), 'filters'=> $filters, 'template'=> $template, 'pagination'=> $pagination, 'total'=> $the_query->max_num_pages );
			wp_reset_query( );
	...
	function traverse_and_build_elements( &$element_template, &$parent, &$css, &$elm_id, $selector_prepend ){
		...
		switch( $element_template[ 'Type' ] ){
			...
			case 'HTML':
				if( $link ) $content = "<a class='wph_auto_a' {$link} {$target}>$content</a>";
				$element_markup = "<{$element_template[ 'Content' ][ 'tag_html' ]} class='wph_element {$classes}' {$type} {$attrs} {$data_elm_id}>{$content}</{$element_template[ 'Content' ][ 'tag_html' ]}>";
				break;
			case 'image':
			...
			case 'ribbon':
				...
				break;
			case 'container':
				$element_markup = "<{$tag_html} class='wph_element {$classes}' {$type} {$attrs} {$link} {$target} {$data_elm_id}></{$tag_html}>";
				break;
			case 'lightbox':
				$element_markup = "<div class='wph_element {$classes}' {$type} {$attrs} {$data_elm_id}></div>";
				break;
			case 'attribute':
		...
		// append to parent
		$element = phpQuery::newDocumentHTML( $element_markup );
		$parent->append( $element );
		$element = $parent->children( '.wph_element' )->filter( ':last' );
		...
		if( ! empty( $element_template[ 'sub_elements' ] ) ){
			...
			foreach( $element_template[ 'sub_elements' ] as $key => &$child_element_template ){
				$this->traverse_and_build_elements( $child_element_template, $element, $css, $elm_id, $selector_prepend ); // now the current element becomes the parent to append to
			}
		}
	...
	public static function process_meta( $mixed ){
		...
		if( 'post_id' === $meta_key ){
			...
			if( count( $parts ) > 1 ){ // word limit set, possibly append string too
				...
				$word_limit = trim( $parts[ 1 ] );
				if( ! is_numeric( $word_limit ) ) die( 'not numeric' );
